...
| S7 | 233 | 52% | +59% | 27.67 | -0.51% |

OOS = Out-of-Sample，意思是「策略開發時沒看過的時間段」，數字相對誠實。回測已扣手續費 0.285% + 證交稅 0.3% + 滑價 0.1%。

> S7 的 60 日漲幅上限 2026-06 由 +25% 放寬到 **+35%**（掃描顯示牛市小賺、熊市退化小，>50% 無效）；上表已是 +35% 的 OOS 結果。
...
4. **RSI < 70** — 還沒過熱
5. **過去 60 天外資+投信淨買進 ≥ 1,000 張（100 萬股）** — 真的有法人在累積
6. **本益比 < 20** 或沒資料 — 不要買貴股
...
4. **股價站上 60 日均線** — 趨勢明確
5. **過去 60 天外資+投信淨買進 ≥ 5,000 張（500 萬股，S4 的 5 倍）** — 法人共識超強
6. **大盤是多頭**
...
1. **過去 60 天外資+投信淨買進 ≥ 2,000 張（200 萬股）** — 法人在累積（門檻比 S4 鬆，因為要抓「早期」）
2. **過去 60 天股價漲幅在 -5% ~ +35% 之間** — 法人在買、股價還沒大漲。如果已經漲超過 35%，代表故事已被市場反映、進場太晚（上限 2026-06 由 +25% 放寬到 +35%）
...
同一天亮燈的候選常有好幾檔。系統會**按「動能 20 日」（過去 20 交易日漲幅）由強到弱排序**，強勢的排前面優先看；**動能 ≤ 0 的標 `⚠️動能未表態`**（價格還沒表態，歷史上這批買了偏賠、可跳過）。

**為什麼是動能、為什麼是 20 日**（樣本外 2023-25 實測）：
- 在「已經是候選」的股票裡再按動能排序，**前半 vs 後半淨報酬差 +0.68pp**；換成法人 60 日 / 三大 20 日排序**完全沒效**（籌碼資訊在篩選階段已用掉）。
- 窗口掃描：3-5 日太短（被短期反轉抵銷）、**20-60 日是穩定平台**，所以取 20（從平台挑整數、不是挑最高那根 → 不算過擬合）。

這是「**簡單勝過複雜**」的實例：同一批因子餵給 GBDT 樹模型，樣本外淨報酬（+0.34%）反而**輸給這條笨動能規則**（+0.60%）。弱訊號的金融資料上，複雜模型容易把噪音當規律 → stockstw 維持規則式、不上 ML。

---

## 📤 出場監控（進場後何時退）

系統不只告訴你「買什麼」，也監控「進場後何時該退」。兩個工具：

| | 策略出場監控 | 個人持股健檢 |
|---|---|---|
| 追蹤 | 系統 S4-S7 訊號出來的單 | 你 `portfolio.yml` 的實際持股（含自己買的） |
| 發送 | 廣播（每日 Telegram） | 個人、不廣播（`python -m analysis.portfolio_health`） |

**出場判斷靠「論點破壞」，不是到價/到天數的硬規則：**

- 🚨 **資金持續撤離**：近 10 日外資「賣超天數 ≥ 6」+ 大幅 → 真的在撤（用「賣超天數」而非「加總」，避免被單一爆量日灌爆又忽略外資已轉買）；+ 散戶接手 / AQS 紅旗 = 倒貨給散戶
- 🚨 **AQS 籌碼崩壞**：階段轉「派發」/ 低分 + 紅旗
- ⚠️ **買力轉弱**：量價同向↓ / 近 5 日法人轉賣

**一條鐵律（回測證明）**：這套籌碼出場**只當提醒、不要機械化照賣**。把它做成自動出場規則回測 → 砍掉 S7 的大贏家尾巴、Sharpe/回撤全變差。它救的是「死錢時間」（及早認賠轉倉），不是「避免賠錢」——散戶接力失敗的賠是策略計劃中的輸單。

**個人健檢的「價格 vs 籌碼」**：跌破 -15% 停損時，若外資仍買超 → 降為「⚠️ 你判斷」而非「🚨 立即出」，把「價格說砍、籌碼說接」的矛盾攤開讓你決定。
...
- **官方 bulk 資料管線**：每日價量／三大法人／月營收改走 TWSE/TPEx 官方 API（單一請求回傳全市場、免 token、無限流），FinMind 只留作財報/PER/興櫃/歷史回補。經逐筆比對與 FinMind 完全一致。每日跑批從 ~4-5 小時降到數分鐘
...
- **美國信用壓力溫度（看盤用，非自動避險）**：報告標頭顯示 `🌡 HY OAS`（FRED 免費）；🟢<3.5 / 🟡<5 / 🟠<7 / 🔴≥7 + 月內擴大警訊。事件研究證實「固收領先股市」（2008 GFC TED 領先 80 天、funding 壓力使台股 6 月內 ≥15% 大跌機率 8%→23%），但**機械化避危機不可行**（誤報率太高、2020 疫情這類外生衝擊零警訊、曲線倒掛是 18 個月前的訊號會洗在末段多頭）→ 只當人工溫度計，跳到 🟠/🔴 才手動縮 S6/S7
...
每日的 cache.db 用 `actions/cache` 在 run 之間保留；cache miss 時自動從最新 GitHub Release 下載 bootstrap 後的 DB 快照，跑完再壓縮上傳回 Release（兩層保險）。
